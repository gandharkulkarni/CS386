...
// console.log(fCalcColTotal(sales,12));
// fCalcTotals(sales);
fCalcAverageTotals(sales);
...
function fCalcAverageTotals(salesArr){
    let year = 2016;
    let month = 1;
    console.log("------------Totals per year------------");
    let sum=0;
    ...
    console.log("------------------------");
    console.log(`Yearly average sales\t\t= $${sum/3}`);
    console.log("------------Totals per Month------------");
    for(let i=0; i<12; i++){
        let monthlyTotal = fCalcColTotal(sales, i);
        console.log(`Total for month ${month}\t\t= $${monthlyTotal.toFixed(2)} (Monthly average sales = $${(monthlyTotal/3).toFixed(2)})`);
        month++;
    }
}
